                    ...
                    @foreach ($conferences as $conference)
                        @if (Carbon::parse($conference->end_date)->isFuture() || Carbon::parse($conference->end_date)->isToday())
                            <div class="g-col-md-4 con-cell">
                                <div class="news-cell">
                                    <div class="event-img">
                                        <img width="auto" height="auto" src="{{ asset('3-images/scso-sgi.webp') }}"
                                            alt="Empowering Women Artisans for Vision Viksit Bharat">
                                    </div>
                                    <h3 class="description">{{Str::limit($conference->description, 200, '....') }}</h3>
                                    <span class="title">({{Str::limit($conference->title, 20, '....') }})</span>
                                    <div class="mt-3">
                                        <div class="call-action "><i class="fa-solid text-bg p-1 fa-calendar-days"></i>
                                            {{ $conference->start_date }}</div>
                                        <div class="call-action mt-1"><a href="{{ route('conference.show', $conference->id) }}"> <i
                                                    class="fa-solid p-1 text-bg fa-arrow-right"></i> Read More</a></div>
                                    </div>
                                </div>
                            </div>
                        @endif
                    ...
                    @foreach ($conferences as $conference)
                        @if (Carbon::parse($conference->end_date)->isPast())
                            <div class="g-col-md-4 con-cell">
                                <div class="news-cell">
                                    <div class="event-img">
                                        <img width="auto" height="auto" src="{{ asset('1-Frontend/image/scso-sgi.webp') }}"
                                            alt="Empowering Women Artisans for Vision Viksit Bharat">
                                    </div>
                                    ...
                                    <div class="call-action"><a href="{{ route('conference.show', 1) }}" class="link-btn"><img
                                                src="" width="24" height="24" loading="lazy"> Read More</a></div>
                                </div>
                            </div>
                        @endif
                    @endforeach
